<template>
  ...
  >
          ...
          >
            <div class="todo-list-card-head" @click="checkIfShowDetail(index)">
              ...
              <div class="todo-list-card-head-opration flex-center">
                <div
                  class="button-rollback flex-center"
                  @click="rollbackMission(index)"
                >
                  回档
                </div>
              </div>
    ...
    <div class="no-data flex-center" v-else>
      ...
      <div class="no-data-text">糟糕，居然没有历史任务·······</div>
    </div>
  </transition>
</template>
  ...
  &-card {
    ...
    &-head {
      ...
      &-opration {
        ...
        .button-rollback {
          height: 60%;
          background-color: rgba(246, 90, 6, 0.5);
          width: 80px;
          border-radius: 5px;
          transition: all ease 0.25s;

          &:hover {
            box-shadow: inset 0px 0px 10px rgba(193, 129, 18, 0.5);
            background-color: rgba(167, 119, 64, 0.5);
          }
        }
      }
    }
